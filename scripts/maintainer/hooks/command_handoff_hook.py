...
MAX_SESSION_DIRS = 24
MAX_COMMAND_RECORDS_PER_SESSION = 250
...
def _trim_session_records(session_dir: Path) -> None:
    ...
    try:
        ...
        return
    if len(records) <= MAX_COMMAND_RECORDS_PER_SESSION:
        return

    keep = records[-MAX_COMMAND_RECORDS_PER_SESSION:]
    log_path.write_text("".join(json.dumps(record) + "\n" for record in keep))
    keep_seq = {int(record.get("seq", -1)) for record in keep}
    for path in session_dir.glob("cmd-*.md"):
        match = re.match(r"cmd-(\d+)-", path.name)
        if match and int(match.group(1)) not in keep_seq:
            path.unlink(missing_ok=True)

    rollup = session_dir / "SESSION_HANDOFF.md"
    created_at = keep[0].get("timestamp") or _now_iso()
    lines = [
        "# Session Handoff",
        "",
        f"- Session key: `{keep[-1].get('session_key', session_dir.name)}`",
    ...
    for record in keep:
        cmd_name = next(
            (
                path.name
                for path in session_dir.glob(f"cmd-{int(record.get('seq', -1)):04d}-*.md")
            ),
            "",
        )
        lines.append(
            f"- `{int(record.get('seq', 0)):04d}` `{record.get('timestamp', '')}` "
            f"`{record.get('tool_name', '')}` `{record.get('preview', '')}` -> `{cmd_name}`"
        )
    rollup.write_text("\n".join(lines) + "\n")
...
def _prune_old_sessions() -> None:
    if not _BASE_DIR.exists():
        return
    session_dirs = [path for path in _BASE_DIR.glob("session-*") if path.is_dir()]
    if len(session_dirs) <= MAX_SESSION_DIRS:
        return
    session_dirs.sort(key=lambda path: path.stat().st_mtime, reverse=True)
    for stale in session_dirs[MAX_SESSION_DIRS:]:
...
def main() -> None:
    ...
    _trim_session_records(session_dir)
    _prune_old_sessions()
