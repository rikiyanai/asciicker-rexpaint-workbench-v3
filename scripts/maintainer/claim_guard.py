...
FL_REF_PATTERN = re.compile(r"\bFL-\d{3,}\b")

# Commit hash candidates must resolve to real commits in this repo.
COMMIT_REF_PATTERN = re.compile(r"\b[0-9a-f]{7,40}\b")


def _has_valid_commit_ref(message: str) -> bool:
    for token in COMMIT_REF_PATTERN.findall(message.lower()):
        try:
...
def check_message(
    ...
    has_commit_ref = _has_valid_commit_ref(message)
    has_evidence = has_fl_ref or has_commit_ref
